...
def get(url: str) -> dict:
    ...
    with requests.get(info_url.format(id=voiceId), headers=headers, timeout=10) as rep:
        if rep.status_code == 200 and rep.json().get("code") == 0:
            info = rep.json()
            userName = info.get("data").get(
                "userVoice").get("userInfo").get("name")
            voiceName = info.get("data").get(
                "userVoice").get("voiceInfo").get("name")
            voiceUrl = info.get("data").get("userVoice").get(
                "voicePlayProperty").get("trackUrl")
